...
export function PlannerPage() {
  ...
  // =========================================================================
  // PROJECTS VIEW (Gantt-style, kept as is)
  // =========================================================================

  const DAY_WIDTH = 3;
  const ganttTotalDays = totalWeeks * 7;
  const ganttWidth = ganttTotalDays * DAY_WIDTH;

  const ganttMonthHeaders: { label: string; left: number; width: number }[] =
    [];
  let cur = new Date(startDate);
  while (cur < endDate) {
    const monthEnd = new Date(cur.getFullYear(), cur.getMonth() + 1, 0);
    const visStart = cur > startDate ? cur : startDate;
    const visEnd = monthEnd < endDate ? monthEnd : endDate;
    const left = daysBetween(startDate, visStart) * DAY_WIDTH;
    const width = (daysBetween(visStart, visEnd) + 1) * DAY_WIDTH;
    ganttMonthHeaders.push({
  ...
  const projectColorMap = useMemo(
    () => new Map(projects.map((p) => [p.id, projectColor(p.id)])),
    [projects],
  );
  ...
  const todayGanttOffset = daysBetween(startDate, today) * DAY_WIDTH;
  const showGanttToday =
    todayGanttOffset >= 0 && todayGanttOffset <= ganttWidth;

  return (
    <div className="flex h-full flex-col">
      {/* Toolbar */}
      <div className="flex items-center justify-between border-b bg-white px-4 py-1.5">
        ...
        <div className="flex items-center gap-1">
          ...
          <select
            value={period}
            onChange={(e) => setPeriod(e.target.value as PeriodKey)}
            className="ml-2 rounded-md border border-gray-300 px-2 py-1 text-xs font-medium text-gray-700 focus:outline-none"
          >
            {PERIODS.map((p) => (
              <option key={p.key} value={p.key}>
                {p.label}
      ...
      <div className="flex-1 overflow-auto">
        <div className="flex min-h-full">
          {/* Left panel */}
          <div className="sticky left-0 z-10 w-56 flex-shrink-0 border-r bg-white">
            <div className="flex h-[60px] items-end border-b px-3 pb-2">
            ...
            {projectRows.map((row) => (
              <div
                key={row.id}
                className="flex h-[40px] items-center border-b px-3 hover:bg-gray-50"
              >
          ...
          <div className="flex-1 overflow-x-auto">
            <div
              style={{ width: ganttWidth, minHeight: '100%' }}
              className="relative"
            >
              <div className="sticky top-0 z-10 h-[60px] border-b bg-white">
                {ganttMonthHeaders.map((mh, i) => (
                  <div
                    key={i}
                    className="absolute top-0 flex h-full items-end border-r border-gray-200 px-2 pb-2"
              ...
              {projectRows.map((row) => (
                <div key={row.id} className="relative h-[40px] border-b">
                  {row.assignments.map((a) => {
                    const aStart = new Date(a.startDate);
                    const aEnd = new Date(a.endDate);
                    const left = daysBetween(startDate, aStart) * DAY_WIDTH;
                    const width =
                      (daysBetween(aStart, aEnd) + 1) * DAY_WIDTH;
                    const color =
                      projectColorMap.get(a.projectId) || '#6366f1';
                    const label =
                      personNameMap.get(a.personId) || 'Person';

                    return (
                      <div
                        key={a.id}
                        className="absolute top-1 flex h-7 cursor-pointer items-center overflow-hidden rounded px-1.5 transition-all hover:brightness-110"
                        style={{
                          left: Math.max(left, 0),
                          width: Math.max(width, 4),
                          backgroundColor: color,
                          opacity: a.isBillable ? 1 : 0.6,
                        }}
                        title={`${label}\n${a.startDate} → ${a.endDate}\n${a.minutesPerDay / 60}h/day`}
                      >
