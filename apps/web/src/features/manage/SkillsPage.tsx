...
import { useSkills, useCreateSkill, useUpdateSkill, useDeleteSkill } from '@/shared/api/hooks';
import type { Skill } from '@/shared/api/hooks';
...
export function SkillsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <PageHeader icon={<Zap className="h-6 w-6" />} title="Skills" count={skills.length} actions={<Button onClick={() => { setAdding(true); setNewName(''); }}><Plus className="h-4 w-4" /> New Skill</Button>} />
      <ListCard search={search} onSearchChange={setSearch} searchPlaceholder="Search skills...">
        <DataTable<Skill> columns={columns} data={filtered} loading={isLoading} emptyMessage="No skills yet." />
        {adding && (
